...
assert.ok(html.includes("Continue with Google or GitHub"), "Workup Studio should use OAuth instead of username/password sign-in.");
assert.ok(html.includes("WORKUP_STUDIO_OAUTH_PROVIDER = \"google\""), "Google should be the configured OAuth provider.");
assert.ok(html.includes("WORKUP_STUDIO_OAUTH_PROVIDER_OPTIONS = Object.freeze([\"google\", \"github\"])"), "GitHub should be supported as an alternate Supabase OAuth provider.");
assert.ok(html.includes("auth/v1/authorize"), "Workup Studio should start Supabase OAuth through the authorize endpoint.");
assert.ok(html.includes("/auth/v1/settings"), "Workup Studio should check enabled Supabase OAuth providers before redirecting.");
...
assert.ok(html.includes("No Supabase OAuth provider is enabled for Workup Studio"), "No-provider errors should be understandable.");
