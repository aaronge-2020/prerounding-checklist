...
function cleanGeneratedScaffoldText(value = "", fallback = "") {
  ...
  text = text
    ...
    .replace(/\bEmergency stabilization route\b/gi, "shock or active bleeding stabilization")
    .replace(/\bRBC transfusion threshold route\b/gi, "RBC transfusion thresholds")
    .replace(/\bIron therapy and diet route\b/gi, "oral iron and diet therapy")
    .replace(/\bITP observation treatment and precautions route\b/gi, "ITP observation and bleeding precautions")
    .replace(/\bLeukemia lymphoma urgent referral route\b/gi, "leukemia/lymphoma urgent referral")
    .replace(/\bHematology handoff and review owner route\b/gi, "hematology handoff owner")
...
function compactWorkupLabel(value = "", fallbackId = "") {
  const clean = displayWorkupLabel(value);
  if (clean.length <= 58) return clean;
  const fromId = cleanText(String(fallbackId || "").replace(/_v1$/i, "").replace(/_/g, " "));
  if (!fromId) return clean;
  return fromId
...
function phraseCandidates(value = "") {
  const text = cleanGeneratedScaffoldText(value).replace(/\s*\([^)]{1,90}\)/g, "").replace(/\.$/, "");
  if (!text) return [];
  const pieces = text
    .split(/\s+\|\s+|;\s*|(?<=[.!?])\s+/)
    .map((piece) => cleanText(piece).replace(/\.$/, ""))
    .filter(Boolean);
  return pieces.length ? pieces : [text];
}

function concisePhrase(value = "", maxLength = 86) {
  const candidates = phraseCandidates(value);
  for (const candidate of candidates) {
    if (candidate.length <= maxLength) return candidate;
    const colon = candidate.indexOf(":");
    if (colon > 12 && colon <= maxLength) return cleanText(candidate.slice(0, colon));
    const comma = candidate.indexOf(",");
    if (comma > 24 && comma <= maxLength) return cleanText(candidate.slice(0, comma));
  }
  return "";
}

function compactArrayText(items = [], fallback = "", maxItems = 4, maxLength = 220) {
  const chosen = [];
  for (const item of unique(items)) {
    const phrase = concisePhrase(item, Math.min(96, maxLength));
    if (!phrase || chosen.includes(phrase)) continue;
    const next = cleanText([...chosen, phrase].join("; "));
  ...
  return cleanText(chosen.join("; ") || fallback);
}

function compactBranchLabel(prefix, items = [], fallback = "", maxLength = 96) {
  const bodyBudget = Math.max(24, maxLength - prefix.length - 2);
  const details = compactArrayText(items, "", 3, bodyBudget);
  const label = cleanText(details ? `${prefix}: ${details}` : fallback || prefix);
  if (label.length <= maxLength) return label;
  const cleanFallback = cleanText(fallback || prefix);
  return cleanFallback.length <= maxLength ? cleanFallback : prefix;
}

function compactEvidenceSummary(items = [], fallback = "", maxItems = 4, maxLength = 220) {
  return compactArrayText(listLabels(items, maxItems * 2), fallback, maxItems, maxLength);
}
...
function buildCompactClinicalPathwayTree(module, sourceById) {
  ...
  const modifierSummary = compactArrayText([
  ...
  ], `${displayLabel} pregnancy, medication, comorbidity, procedure, assay, and treatment-safety modifiers`, 5, 240);
  const managementActionSummary = compactArrayText([
    ...treatmentModifierLabels,
    ...dispositionModifierLabels,
    ...diseaseModifierLabels.slice(0, 5),
    ...clinicalSafetySummary
  ], treatmentSummary, 6, 360);
  const mimicSummary = compactArrayText(
  ...
  const branchDetail = (value = "") => {
    const prefix = `${displayLabel}:`;
    let text = cleanGeneratedScaffoldText(value);
    if (text.toLowerCase().startsWith(prefix.toLowerCase())) {
      text = cleanText(text.slice(prefix.length));
    }
    return text
      .replace(/^(?:immediate stabilization or escalation trigger|treatment or disposition for high-risk or confirmed disease)\s*:\s*/i, "")
      .replace(/\s+\.\s*$/g, "")
      .trim();
  };
  ...
  const displayUrgent = compactArrayText(branchDetails([
    ...
    ...thresholdExamples.slice(0, 2)
  ]), "unstable physiology, high-acuity threshold, or monitored-care rule", 5, 240);
  const displayMimics = compactArrayText(
    branchDetails(visibleListLabels(differentials, 4)),
    "competing diagnosis better explains the presentation",
    3,
    220
  );
  const rootThresholdDisplay = compactArrayText(branchDetails(thresholdExamples), "cited diagnostic cutoffs", 4, 140);
  const rootMimicDisplay = compactArrayText(
    branchDetails(visibleListLabels(differentials, 3)),
    "competing diagnosis fits better",
    2,
    120
  );
  const displayModifiers = compactArrayText(branchDetails([
    ...diseaseModifierLabels.slice(0, 3),
    ...dispositionModifierLabels.slice(0, 2),
    ...treatmentModifierLabels.slice(0, 2),
    ...clinicalSafetySummary.slice(0, 2)
  ]), "pregnancy, medication, organ-function, dosing, procedure, or assay modifier", 5, 220);
  const displayMonitoring = compactArrayText(branchDetails([
    ...thresholdExamples.slice(0, 2),
    ...visibleListLabels(tests, 2),
    "treatment response",
    "adverse effects",
    "disposition readiness"
  ]), "response, adverse effects, and disposition readiness", 5, 220);
  const displayWorsening = compactArrayText(branchDetails([
    ...visibleRedFlagLabels.slice(0, 2),
    ...thresholdExamples.slice(0, 2),
    ...visibleListLabels(tests, 1),
  ...
  const managementEdgeDisplay = compactArrayText(managementItems, diseaseBranchLabel("treatment decision"), 4, 230);
  const rootManagementDisplay = compactArrayText(managementItems, diseaseBranchLabel("treatment decision"), 3, 180);
  const edgeLabels = {
    ...
    classification: `Cutoff check: ${displayThresholds}`,
    ...
    safetySelected: `Safety modifier: ${displayModifiers}`,
    review: `Review trigger: ${displayModifiers}`,
    ...
    followup: `Follow-up: result owner, interval, and return precautions`
  };

  const followupEndpoint = endpoint({
    id: `${prefix}_followup_endpoint`,
    label: branchLabels.followup,
    edgeLabel: edgeLabels.followup,
    sourceIds: managementSources,
    criteria: criteria(`${prefix}_followup_criteria`, `Use after ${label} is stable, actionable results have an owner, and return precautions are recorded.`, ["symptoms", "vitals", "labs", "imaging_results", "medications", "follow_up_access", "workup_findings"], managementSources),
  ...
  const worseningEndpoint = endpoint({
    ...
    action: `Review ${displayWorsening}; reassess mimics and complications; move to ED, admission, or specialty review when the cited trend or finding worsens.`,
  ...
  const safetyDecision = decision({
    ...
    action: `Apply ${displayLabel}-specific safety modifiers before final therapy, procedure, dose, or care-location choice.`,
  ...
  const urgentEndpoint = endpoint({
    ...
    action: `Send to ED, admission, or monitored specialty care for ${displayUrgent}; start stabilization and diagnosis-specific emergency treatment before routine outpatient management.`,
    endpointType: "escalation_disposition",
    guidelineCutoffs: criteriaRows
  });
  ...
  const classificationDecision = decision({
    ...
    label: branchLabels.classification,
  ...
  const dataBundle = actionNode({
    ...
    action: `Obtain together: ${displayDiagnosticData}.`,
    parallelActions: unique([...visibleListLabels(tests, 6), ...clinicalSafetySummary.slice(0, 4), ...criteriaRows.flatMap((row) => row.data_needed || []).slice(0, 6)]),
  ...
  const root = decision({
    ...
    action: `Send unstable patients to urgent care. Compare ${rootThresholdDisplay}. Treat confirmed or likely ${displayLabel} with ${rootManagementDisplay}. If ${rootMimicDisplay}, leave this pathway and document the competing diagnosis.`,
...
function buildAdultSepsisClinicalPathwayTree(module, sourceById) {
  ...
  const shockIcuEndpoint = endpoint({
    ...
    label: "Septic shock or hypoperfusion",
  ...
  const persistentHypoperfusionEndpoint = endpoint({
    ...
    label: "Persistent hypoperfusion",
  ...
  const probableSepsisTreatmentEndpoint = endpoint({
    ...
    label: "Probable sepsis treatment",
  ...
  const possibleSepsisRapidEvaluationEndpoint = endpoint({
    ...
    label: "Possible sepsis rapid evaluation",
  ...
  const mimicEndpoint = endpoint({
    ...
    label: "Noninfectious mimic or alternate source",
  ...
  const sourceControlEndpoint = endpoint({
    ...
    label: "Source-control procedure needed",
  ...
  const deescalateEndpoint = endpoint({
    ...
    label: "Antimicrobial de-escalation criteria",
  ...
  const probablePersistentEndpoint = endpoint({
    ...
    label: "Probable sepsis worsening on reassessment",
  ...
  const probableDeescalateEndpoint = endpoint({
    ...
    label: "Probable sepsis antimicrobial de-escalation",
  ...
  const probableDischargeEndpoint = endpoint({
    ...
    label: "Probable sepsis disposition and safety-net",
  ...
  const dischargeEndpoint = endpoint({
    ...
    label: "Sepsis follow-up and safety-net",
  ...
  const shockAction = actionNode({
    ...
    label: "Septic shock resuscitation route",
  ...
  const probableSepsisAction = actionNode({
    ...
    label: "Probable sepsis treatment route",
  ...
  const possibleSepsisAction = actionNode({
    ...
    label: "Possible sepsis evaluation route",
  ...
  const initialAssessment = actionNode({
    ...
    label: "Initial fever/infection assessment",
...
function buildAdultDkaHhsClinicalPathwayTree(module, sourceById) {
  ...
  const potassiumHoldEndpoint = endpoint({
    ...
    label: "Potassium hazard before insulin",
  ...
  const dkaTransitionEndpoint = endpoint({
    ...
    label: "Adult DKA transition and recurrence prevention",
  ...
  const dkaAction = actionNode({
    ...
    label: "Adult DKA treatment route",
  ...
  const hhsTransitionEndpoint = endpoint({
    ...
    label: "Adult HHS de-escalation readiness",
  ...
  const hhsAction = actionNode({
    ...
    label: "Adult HHS treatment route",
  ...
  const euglycemicSpecialEndpoint = endpoint({
    ...
    label: "Euglycemic DKA or special-population review",
  ...
  const noncrisisEndpoint = endpoint({
    ...
    label: "Non-crisis hyperglycemia",
  ...
  const alternateEndpoint = endpoint({
    id: `${prefix}_alternate_endpoint`,
    label: "DKA/HHS mimic or alternate diagnosis",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose adult hyperglycemia branch",
  ...
  const initialAssessment = actionNode({
    ...
    label: "Initial adult hyperglycemia assessment",
...
function buildPediatricDkaHhsClinicalPathwayTree(module, sourceById) {
  ...
  const cerebralEdemaEndpoint = endpoint({
    ...
    label: "Suspected cerebral edema",
  ...
  const shockBolusEndpoint = endpoint({
    ...
    label: "Shock or cardiovascular compromise",
  ...
  const potassiumAnuriaEndpoint = endpoint({
    ...
    label: "Potassium danger or anuria",
  ...
  const pediatricHhsEndpoint = endpoint({
    ...
    label: "Pediatric HHS or mixed DKA/HHS",
  ...
  const transitionEndpoint = endpoint({
    ...
    label: "Pediatric DKA transition and recurrence prevention",
  ...
  const glucoseFluidsEndpoint = endpoint({
    ...
    label: "DKA glucose-fluid adjustment",
  ...
  const alternateEndpoint = endpoint({
    id: `${prefix}_alternate_endpoint`,
    label: "Pediatric DKA/HHS mimic or no crisis",
  ...
  const dkaAction = actionNode({
    id: `${prefix}_dka_action`,
    label: "Pediatric DKA treatment route",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose pediatric hyperglycemia branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: "Initial pediatric hyperglycemia assessment",
...
function buildCongenitalAdrenalHyperplasiaClinicalPathwayTree(module, sourceById) {
  ...
  const crisisEndpoint = endpoint({
    ...
    label: "Salt-wasting CAH or adrenal crisis",
  ...
  const newbornDiagnosticEndpoint = endpoint({
    ...
    label: "Newborn/classic CAH confirmation",
  ...
  const afterInfancyDiagnosticEndpoint = endpoint({
    ...
    label: "After-infancy CAH classification",
  ...
  const classicTreatmentEndpoint = endpoint({
    ...
    label: "Classic CAH replacement plan",
  ...
  const nonclassicTreatmentEndpoint = endpoint({
    ...
    label: "Symptomatic nonclassic CAH treatment",
  ...
  const nonclassicDeescalationEndpoint = endpoint({
    ...
    label: "Nonclassic CAH no daily-steroid route",
  ...
  const crinecerfontEndpoint = endpoint({
    ...
    label: "Crinecerfont adjunct review",
  ...
  const pregnancyDsdReviewEndpoint = endpoint({
    ...
    label: "Pregnancy, fertility, or DSD review",
  ...
  const monitoringEndpoint = endpoint({
    ...
    label: "CAH monitoring and overtreatment checks",
    ...
    action: "For patients 18 months or younger, schedule close follow-up during the first 3 months and every 3 months thereafter; after 18 months, evaluate about every 4 months while growing. Track growth velocity, weight, BP, virilization or Cushingoid features, sodium, potassium, renin, and timed hormones. Adults need at least annual exam with BP/BMI and biochemical monitoring. Use local endocrine policy for testicular adrenal rest tumor ultrasound. Escalate urgently for vomiting, dehydration, hypoglycemia, electrolyte instability, or inability to take steroids.",
    endpointType: "monitoring_reassessment",
    guidelineCutoffs: criteriaRows,
  ...
  const safetyNetEndpoint = endpoint({
    ...
    label: "CAH follow-up and sick-day safety-net",
    ...
    action: "Document ED/admission/ICU for crisis or unstable electrolytes/glucose, urgent pediatric endocrine follow-up for newborn positives or unclear classification, or outpatient endocrine follow-up when meds and monitoring are secure. Provide written sick-day rules, teach parenteral hydrocortisone injection, confirm emergency kit, medical alert ID/steroid card, refills, pending-result owner, and return precautions for fever >38.5 C, vomiting, dehydration, syncope, severe weakness, confusion, hypoglycemia, inability to keep steroids down, trauma, surgery, labor, crinecerfont hypersensitivity, worsening virilization, or poor growth.",
    endpointType: "safety_net_instruction",
    guidelineCutoffs: criteriaRows,
...
function buildAdrenalInsufficiencyClinicalPathwayTree(module, sourceById) {
  ...
  const crisisEndpoint = endpoint({
    id: `${prefix}_adrenal_crisis_endpoint`,
    label: "Adrenal crisis or severe adrenal insufficiency",
  ...
  const primaryConfirmedEndpoint = endpoint({
    ...
    label: "Primary adrenal insufficiency replacement",
  ...
  const borderlineEndpoint = endpoint({
    ...
    label: "Borderline cortisol or assay review",
  ...
  const mimicEndpoint = endpoint({
    id: `${prefix}_mimic_exclusion_endpoint`,
    label: "Adrenal mimic or competing pattern",
  ...
  const specialPopulationEndpoint = endpoint({
    ...
    label: "Pregnancy, pediatric, or stress-dose review",
  ...
  const monitoringEndpoint = endpoint({
    id: `${prefix}_monitoring_reassessment_endpoint`,
    label: "Adrenal therapy monitoring",
  ...
  const transitionEndpoint = endpoint({
    ...
    label: "Adrenal crisis transition and sick-day plan",
  ...
  const chronicAction = actionNode({
    ...
    label: "Chronic adrenal replacement plan",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose adrenal insufficiency branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: "Initial adrenal insufficiency assessment",
...
function buildHypopituitarismClinicalPathwayTree(module, sourceById) {
  ...
  const apoplexyCrisisEndpoint = endpoint({
    ...
    label: "Pituitary apoplexy or adrenal crisis",
  ...
  const centralAiEndpoint = endpoint({
    ...
    label: "Central adrenal replacement first",
  ...
  const centralHypothyroidEndpoint = endpoint({
    ...
    label: "Central hypothyroidism treatment",
  ...
  const diEndpoint = endpoint({
    ...
    label: "Central DI and DDAVP safety",
  ...
  const gonadalGhProlactinEndpoint = endpoint({
    ...
    label: "Gonadal, GH, and prolactin-axis review",
  ...
  const mimicEndpoint = endpoint({
    id: `${prefix}_mimic_exclusion_endpoint`,
    label: "Pituitary-axis mimic or alternate diagnosis",
  ...
  const monitoringEndpoint = endpoint({
    ...
    label: "Pituitary replacement monitoring",
  ...
  const followupEndpoint = endpoint({
    ...
    label: "Hypopituitarism follow-up and safety-net",
  ...
  const replacementAction = actionNode({
    ...
    label: "Stable hypopituitarism replacement sequence",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose hypopituitarism branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: "Initial hypopituitarism assessment",
...
function buildGestationalDiabetesClinicalPathwayTree(module, sourceById) {
  ...
  const urgentEndpoint = endpoint({
    ...
    label: "Pregnancy diabetes emergency",
  ...
  const diagnosticEndpoint = endpoint({
    ...
    label: "GDM or overt diabetes classification",
  ...
  const treatmentEndpoint = endpoint({
    ...
    label: "GDM nutrition, monitoring, or insulin plan",
  ...
  const mimicEndpoint = endpoint({
    ...
    label: "Pregnancy glucose mimic or overt diabetes",
  ...
  const postpartumEndpoint = endpoint({
    ...
    label: "Postpartum GDM follow-up and prevention",
  ...
  const managementAction = actionNode({
    ...
    label: "Gestational diabetes management plan",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose gestational diabetes branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: "Initial gestational diabetes assessment",
    ...
    action: "Measure BP, HR, weight, mental status, and fetal symptoms when gestational age applies. Obtain gestational age, prior diabetes/GDM status, one-step or two-step testing strategy, exact OGTT or screen values, fasting/postprandial glucose log, ketones/electrolytes/anion gap if vomiting or severely hyperglycemic, medication/allergy list, nutrition/activity plan, and postpartum delivery date if delivered.",
...
function buildDiabetesMellitusClinicalPathwayTree(module, sourceById, diabetesType) {
  ...
  const shortDiabetesLabel = isType1 ? "Type 1 diabetes" : "Type 2 diabetes";
  ...
  const crisisEndpoint = endpoint({
    ...
    label: `${shortDiabetesLabel} crisis or severe hypoglycemia`,
  ...
  const diagnosisEndpoint = endpoint({
    ...
    label: `${shortDiabetesLabel} diagnosis and type classification`,
  ...
  const treatmentEndpoint = endpoint({
    ...
    label: isType1 ? "Type 1 insulin and sick-day plan" : "Type 2 medication and insulin-intensity plan",
    edgeLabel: isType1
  ...
  const riskModifierEndpoint = endpoint({
    ...
    label: isType1 ? "Type 1 pregnancy, device, or access review" : "Type 2 cardiorenal or access review",
    edgeLabel: isType1
  ...
  const followupEndpoint = endpoint({
    ...
    label: `${shortDiabetesLabel} monitoring and safety-net`,
  ...
  const mimicEndpoint = endpoint({
    ...
    label: `${shortDiabetesLabel} mimic or alternate glucose pathway`,
  ...
  const managementAction = actionNode({
    id: `${prefix}_management_action`,
    label: `${shortDiabetesLabel} stable management plan`,
    ...
    action: "Use diagnostic classification, treatment intensity, medication/device safety, kidney/cardiovascular risk, hypoglycemia prevention, and follow-up as one plan.",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: `Choose ${shortDiabetesLabel} branch`,
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: `Initial ${shortDiabetesLabel} assessment`,
...
function buildPrediabetesClinicalPathwayTree(module, sourceById) {
  ...
  const preventionEndpoint = endpoint({
    ...
    label: "Lifestyle prevention plan",
  ...
  const metforminEndpoint = endpoint({
    ...
    label: "High-risk metformin prevention review",
  ...
  const mimicEndpoint = endpoint({
    id: `${prefix}_mimic_endpoint`,
    label: "Prediabetes mimic or normal glycemia",
  ...
  const followupEndpoint = endpoint({
    id: `${prefix}_followup_safety_endpoint`,
    label: "Prediabetes follow-up and safety-net",
  ...
  const managementAction = actionNode({
    id: `${prefix}_management_action`,
    label: "Prediabetes prevention plan",
    ...
    action: "Use prevention thresholds, DPP eligibility, metformin risk criteria, cardiometabolic risk factors, and surveillance as one plan.",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose prediabetes branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: "Initial prediabetes assessment",
...
function buildMetabolicSyndromeClinicalPathwayTree(module, sourceById) {
  ...
  const diagnosisEndpoint = endpoint({
    ...
    label: "Metabolic syndrome criteria met",
  ...
  const riskReductionEndpoint = endpoint({
    ...
    label: "Metabolic risk-reduction plan",
    edgeLabel: "Risk-reduction criteria: metabolic syndrome confirmed or near-threshold high-risk pattern without urgent diabetes/BP/ASCVD symptoms",
  ...
  const mimicEndpoint = endpoint({
    id: `${prefix}_mimic_endpoint`,
    label: "Metabolic syndrome mimic or fewer criteria",
  ...
  const followupEndpoint = endpoint({
    id: `${prefix}_followup_safety_endpoint`,
    label: "Metabolic syndrome follow-up and safety-net",
  ...
  const managementAction = actionNode({
    id: `${prefix}_management_action`,
    label: "Metabolic syndrome management plan",
    ...
    action: "Count all five criteria and treat active risks rather than using the label alone.",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose metabolic syndrome branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment`,
    label: "Initial metabolic syndrome assessment",
...
function buildPrimaryAldosteronismClinicalPathwayTree(module, sourceById) {
  ...
  const emergencyEndpoint = endpoint({
    ...
    label: "Hypertensive emergency or dangerous hypokalemia",
  ...
  const repeatEndpoint = endpoint({
    ...
    label: "Confounded PA screen or mimic",
  ...
  const overtEndpoint = endpoint({
    ...
    label: "Overt PA treatment route",
  ...
  const suppressionEndpoint = endpoint({
    ...
    label: "Intermediate PA confirmation",
  ...
  const lateralizationEndpoint = endpoint({
    ...
    label: "PA lateralization for adrenalectomy",
  ...
  const medicalEndpoint = endpoint({
    ...
    label: "PA medical therapy and monitoring",
  ...
  const positiveAction = actionNode({
    ...
    label: "Positive PA screen route",
  ...
  const initialAssessment = actionNode({
    ...
    action: "Measure repeat BP and target-organ symptoms; obtain potassium, aldosterone, PRA or DRC, ARR with units, assay method, renal function, pregnancy status, medication-interference history, surgery preference, and follow-up access.",
...
function buildPheochromocytomaClinicalPathwayTree(module, sourceById) {
  ...
  const crisisEndpoint = endpoint({
    ...
    label: "PPGL crisis or hypertensive emergency",
  ...
  const borderlineEndpoint = endpoint({
    ...
    label: "Borderline PPGL result or mimic",
  ...
  const localizationEndpoint = endpoint({
    ...
    label: "Confirmed PPGL localization/genetic review",
  ...
  const preopEndpoint = endpoint({
    ...
    label: "Functional PPGL pre-op blockade",
  ...
  const postopEndpoint = endpoint({
    ...
    label: "PPGL follow-up and lifelong surveillance",
  ...
  const positiveAction = actionNode({
    ...
    label: "Positive PPGL biochemistry route",
  ...
  const initialAssessment = actionNode({
    ...
    action: "Measure repeat BP, orthostatic BP when safe, HR/rhythm, oxygenation, and mental status. Obtain plasma free or urinary fractionated metanephrines with ULN, sampling position/reference interval, medication/diet triggers, renal function, glucose if ill or post-op, pregnancy status, adrenal/extra-adrenal imaging context, hereditary risk, and surgery or surveillance status.",
...
function buildAdultChestPainClinicalPathwayTree(module, sourceById) {
  ...
  const lifeThreateningMimicEndpoint = endpoint({
    ...
    label: "Non-ACS chest pain emergency",
  ...
  const lowRiskEndpoint = endpoint({
    ...
    label: "Low-risk chest pain discharge",
  ...
  const intermediateEndpoint = endpoint({
    ...
    label: "Intermediate-risk testing or observation",
  ...
  const testingSafetyEndpoint = endpoint({
    ...
    label: "Chest-pain testing safety review",
  ...
  const worseningEndpoint = endpoint({
    id: `${prefix}_worsening_endpoint`,
    label: "Escalating chest pain on reassessment",
  ...
  const deescalateEndpoint = endpoint({
    id: `${prefix}_deescalation_endpoint`,
    label: "Low-risk ACS de-escalation",
  ...
  const followupEndpoint = endpoint({
    id: `${prefix}_followup_safety_endpoint`,
    label: "Chest-pain follow-up and safety-net",
  ...
  const alternateEndpoint = endpoint({
    ...
    label: "Alternate noncardiac diagnosis",
  ...
  const reassessmentBundle = actionNode({
    ...
    label: "Monitor symptoms, ECG, troponin, and disposition",
  ...
  const riskDecision = decision({
    id: `${prefix}_risk_decision`,
    label: "Choose chest-pain risk branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment_bundle`,
    label: "Initial chest-pain assessment",
...
function buildPediatricFeverSepsisClinicalPathwayTree(module, sourceById) {
  ...
  const septicShockEndpoint = endpoint({
    ...
    action: "Escalate immediately to emergency pediatric critical care or transfer. Measure lactate, obtain blood cultures before antibiotics if this will not substantially delay treatment, give broad-spectrum antibiotics ideally within 1 hour, give glucose-free crystalloid 10-20 mL/kg with reassessment after each bolus, stop for resolution or overload, use vasoactive medication via peripheral IV rather than delaying for central access when shock persists, seek source control, avoid etomidate, and after resuscitation target SpO2 88-92% unless another condition requires a different target.",
    endpointType: "escalation_disposition",
    guidelineCutoffs: criteriaRows
  });
  ...
  const febrileInfantEndpoint = endpoint({
    ...
    action: "For 0-21 days, obtain blood, urine, and CSF studies/cultures and hospitalize with empiric broad-spectrum antibiotics for 24-48 hours while cultures are pending. For 22-28 days, obtain blood culture, inflammatory markers, UA and urine culture if UA positive; abnormal markers, positive UA, or concerning CSF route to CSF evaluation, admission, and IV antibiotics, while outpatient observation requires normal urine/CSF/markers, reliable communication/transport, and reevaluation within 24 hours. For 29-60 days, obtain UA/culture if positive, blood culture, ANC/CRP/procalcitonin, defer CSF only when low-risk logic permits, and assign culture follow-up through final result.",
    endpointType: "diagnostic_step",
    guidelineCutoffs: criteriaRows
  });
  ...
  const initialAssessment = actionNode({
    ...
    action: "Assess age, corrected age when relevant, temperature value/method/duration, caregiver concern, behavior, mental state, work of breathing, SpO2/oxygen, RR/HR/BP, perfusion, urine output, hydration, rash, meningitis/HSV features, respiratory/urinary/GI/skin/CNS/joint/travel/line source, immunization/recent vaccine timing, prematurity/chronic illness/immunocompromise, pregnancy status, meds/allergies, weight, culture/lactate timing when sepsis is possible, infant UA/culture/ANC/CRP/procalcitonin/CSF decision, antibiotics already given, and follow-up reliability.",
...
function buildPediatricChestPainSyncopeClinicalPathwayTree(module, sourceById) {
  ...
  const evidenceLabels = {
    tests: compactArrayText(visibleListLabels(tests, 8), "targeted ECG/testing", 4, 220),
    redFlags: compactArrayText(visibleListLabels(redFlags, 8), "cardiac, respiratory, syncope, or family-history red flags", 4, 220),
    differentials: compactArrayText(visibleListLabels(differentials, 8), "respiratory, PE, seizure, toxin, anemia, pregnancy, or benign chest pain mimics", 4, 220),
    dispositions: compactArrayText(visibleListLabels(dispositions, 8), "activity restriction, follow-up, and return precautions", 4, 220),
    safety: compactArrayText(visibleSafetyLabels, "medication, pregnancy, and access modifiers", 4, 220)
  };
  const criteriaRows = [
    criterion({
      id: "peds_chest_pain_low_base_rate_and_targeted_testing",
  ...
  const cardiacEscalationEndpoint = endpoint({
    ...
    label: "Cardiac red flags or abnormal ECG",
    edgeLabel: "Cardiac risk: exertional pain/syncope, palpitations with syncope, poor perfusion, abnormal cardiac exam, family sudden-death risk, QRS >0.12 sec, QTc <=340 or >450 ms, ST shift >2 mm, WPW/long QT/wide-complex rhythm, or HCM pattern",
  ...
  const respiratoryEndpoint = endpoint({
    ...
    label: "Respiratory, PE, or trauma chest pain",
    edgeLabel: "Respiratory/PE branch: hypoxia, respiratory distress, hemoptysis, pleuritic pain, fever/cough, wheeze, unilateral decreased air entry, trauma, PE risk, or foreign-body concern",
    sourceIds: rchChest,
  ...
  const orthostaticSyncopeEndpoint = endpoint({
    ...
    label: "Vasovagal, orthostatic, or POTS syncope",
  ...
  const lowRiskChestPainEndpoint = endpoint({
    ...
    label: "Low-risk pediatric chest pain",
  ...
  const seizureMimicEndpoint = endpoint({
    ...
    label: "Syncope mimic or neurologic event",
  ...
  const specialReviewEndpoint = endpoint({
    id: `${prefix}_special_population_review_endpoint`,
    label: "Pregnancy, toxin, VTE, or cardiac-history review",
  ...
  const worseningEndpoint = endpoint({
    id: `${prefix}_worsening_endpoint`,
    label: "Escalating chest pain/syncope findings",
  ...
  const followupEndpoint = endpoint({
    id: `${prefix}_followup_safety_endpoint`,
    label: "Chest/syncope follow-up and safety-net",
    edgeLabel: "Disposition branch: cause identified or serious causes addressed, follow-up arranged, activity advice documented, pending results assigned, and family return precautions given",
    sourceIds,
  ...
  const monitoringBundle = actionNode({
    id: `${prefix}_monitoring_bundle`,
    label: "Monitor ECG, vitals, symptoms, and readiness",
  ...
  const classificationDecision = decision({
    id: `${prefix}_classification_decision`,
    label: "Choose pediatric chest/syncope branch",
  ...
  const initialAssessment = actionNode({
    id: `${prefix}_initial_assessment_bundle`,
    label: "Initial chest pain/syncope assessment",
    edgeLabel: "Pediatric chest/syncope assessment: age, exertion, syncope/palpitations, family history, vitals/orthostatics, cardiac/respiratory exam, and targeted ECG/testing",
    sourceIds,
...
function buildPediatricMskLimpHotJointClinicalPathwayTree(module, sourceById) {
  ...
  const visibleSafetyLabels = visibleListLabels(safetyChecks, 8)
    .filter((text) => !/^(Measure|Document)\b|blood pressure|heart rate|respiratory rate|oxygen saturation|temperature|pain score/i.test(text));
  const evidenceLabels = {
    tests: compactArrayText(visibleListLabels(tests, 8), "targeted labs, cultures, X-ray, ultrasound, MRI, or aspiration", 4, 220),
    redFlags: compactArrayText(visibleListLabels(redFlags, 8), "sepsis, septic arthritis, fracture, safeguarding, or systemic red flags", 4, 220),
    differentials: compactArrayText(visibleListLabels(differentials, 8), "infection, injury, hip disease, systemic, referred, or low-risk limp mimics", 4, 220),
    dispositions: compactArrayText(visibleListLabels(dispositions, 8), "weight-bearing plan, specialty follow-up, and return precautions", 4, 220),
    safety: compactArrayText(visibleSafetyLabels, "host, pregnancy, medication, antimicrobial, and follow-up modifiers", 4, 220)
  };
  const criteriaRows = cutoffCriteriaForModule(module);
