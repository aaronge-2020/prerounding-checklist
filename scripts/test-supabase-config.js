...
assert.ok(deployScript.includes("Command output tail"), "Deploy command should surface the failing command output tail.");
assert.ok(deployScript.includes("GITHUB_STEP_SUMMARY"), "Deploy command should write GitHub step summaries for hidden deploy logs.");
assert.ok(deployScript.includes("::error title=Supabase Workup Studio deploy failed"), "Deploy command should emit GitHub annotations for hidden deploy logs.");
...
assert.ok(html.includes("visibilityMs > WORKUP_STUDIO_PUBLIC_VISIBILITY_TARGET_MS"), "Public catalog verification should fail when fresh-device visibility exceeds the 60-second target.");
...
assert.ok(html.includes("applyWorkupStudioPublicCatalogVerification"), "Successful reviewer publish should persist public-catalog verification evidence on the change set.");
assert.ok(html.includes("publicCatalogVerification: verification"), "Verified publish evidence should record the public-catalog proof in imported_evidence.");
assert.ok(html.includes("sourceToLiveMs"), "Source-backed publishes should measure source-to-live time.");
assert.ok(html.includes("sourceToLiveWithinTarget"), "Source-backed publishes should record whether the under-10-minute target was met.");
assert.ok(html.includes("imported_evidence: importedEvidence"), "Public-catalog verification evidence should be patched back to Supabase.");
assert.ok(html.includes("Public catalog verified for fresh devices in ${publicCatalogVerification.visibilityMs} ms"), "Successful reviewer publish should tell the reviewer the measured fresh-device catalog latency.");
