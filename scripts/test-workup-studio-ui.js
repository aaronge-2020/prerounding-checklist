...
try {
  ...
  const addedPhysicalExamItem = `Studio test physical exam perfusion item ${Date.now()}`;
  const promptPersistenceSentinel = `PROMPT_PERSISTENCE_${Date.now()}`;
  await setTextareaValue(page, "#workupStudioOpenEvidencePromptOutput", `${physicalExamPrompt}\n\nReviewer focus: ${promptPersistenceSentinel}`);
  const physicalExamCountBeforeAdd = await page.locator("#workupStudioItemList .studio-item-button").count();
  await page.selectOption("#workupStudioAddItemGroupSelect", "requiredExam");
  await page.click("#workupStudioAddItemButton");
  await page.waitForFunction((before) => document.querySelectorAll("#workupStudioItemList .studio-item-button").length > before, physicalExamCountBeforeAdd);
  await page.fill("#workupStudioItemLabelInput", addedPhysicalExamItem);
  await page.fill("#workupStudioItemPrimaryInput", addedPhysicalExamItem);
  await page.fill("#workupStudioItemRationaleInput", "Added by the browser test to prove physical exam authoring persists.");
  await page.click("#workupStudioSaveItemDraftButton");
  await page.waitForFunction((sentinel) => (document.querySelector("#workupStudioItemList")?.textContent || "").includes(sentinel), addedPhysicalExamItem);
  await waitForCondition(() => supabaseRequests.postedRows.some((row) => (
    row.section_key === "physical_exam" && JSON.stringify(row.after_snapshot || {}).includes(addedPhysicalExamItem)
  )), "Supabase physical exam add-item draft insert");

  await page.locator("#workupStudioSectionTabs button", { hasText: "History questions" }).click();
  await page.waitForSelector("#workupStudioItemLabelInput");
  const addedHistoryQuestion = `Do you have any new medicine access or discharge safety concern ${Date.now()}?`;
  const historyCountBeforeAdd = await page.locator("#workupStudioItemList .studio-item-button").count();
  await page.selectOption("#workupStudioAddItemGroupSelect", "requiredQuestions");
  await page.click("#workupStudioAddItemButton");
  await page.waitForFunction((before) => document.querySelectorAll("#workupStudioItemList .studio-item-button").length > before, historyCountBeforeAdd);
  await page.fill("#workupStudioItemLabelInput", addedHistoryQuestion);
  await page.fill("#workupStudioItemPrimaryInput", addedHistoryQuestion);
  await page.fill("#workupStudioItemRationaleInput", "Added by the browser test to prove history authoring persists.");
  await page.click("#workupStudioSaveItemDraftButton");
  await page.waitForFunction((sentinel) => (document.querySelector("#workupStudioItemList")?.textContent || "").includes(sentinel), addedHistoryQuestion);
  await waitForCondition(() => supabaseRequests.postedRows.some((row) => (
    row.section_key === "history_questions" && JSON.stringify(row.after_snapshot || {}).includes(addedHistoryQuestion)
  )), "Supabase history add-item draft insert");

  await page.goto(`${baseUrl}/index.html?workupStudioPersistenceReload=${Date.now()}`);
  await page.waitForFunction(() => document.body.dataset.view === "vaultAccess");
  await page.click("#demoCaseButton");
  await page.waitForFunction(() => document.body.dataset.view !== "vaultAccess");
  await page.click('button[data-view-target="studio"]');
  ...
  await page.waitForSelector("#workupStudioItemLabelInput");
  await page.waitForFunction((sentinel) => (document.querySelector("#workupStudioItemList")?.textContent || "").includes(sentinel), addedPhysicalExamItem);
  const persistedPrompt = await page.inputValue("#workupStudioOpenEvidencePromptOutput");
  assert.match(persistedPrompt, new RegExp(promptPersistenceSentinel), "Edited section prompt should persist after reload.");

  await page.click("#sidebarPatientRosterButton");
  await page.waitForSelector("#workspaceView:not([hidden])");
  await page.locator('button[data-patient-tab="checklist"]').click();
  await page.click("#workspaceOpenBedsideChecklistButton");
  await page.waitForSelector("#checklistSections .checklist-row");
  const rebuiltChecklistText = await page.textContent("#checklistSections");
  assert.ok(rebuiltChecklistText.includes(addedHistoryQuestion.replace(/\?$/, "")), "Patient checklist should include the locally added history question after reload.");
  assert.ok(rebuiltChecklistText.includes(addedPhysicalExamItem), "Patient checklist should include the locally added physical exam item after reload.");

  await page.click('button[data-view-target="studio"]');
  await page.waitForSelector("#studioView:not([hidden])");
  await page.locator("#workupStudioSectionTabs button", { hasText: "History questions" }).click();
  await page.waitForFunction((sentinel) => (document.querySelector("#workupStudioItemList")?.textContent || "").includes(sentinel), addedHistoryQuestion);
  await page.locator("#workupStudioItemList .studio-item-button", { hasText: addedHistoryQuestion }).click();
  await page.click("#workupStudioDeleteItemButton");
  await page.waitForFunction((sentinel) => !(document.querySelector("#workupStudioItemList")?.textContent || "").includes(sentinel), addedHistoryQuestion);
  await page.goto(`${baseUrl}/index.html?workupStudioDeleteReload=${Date.now()}`);
  await page.waitForFunction(() => document.body.dataset.view === "vaultAccess");
  await page.click("#demoCaseButton");
  await page.waitForFunction(() => document.body.dataset.view !== "vaultAccess");
  await page.click('button[data-view-target="studio"]');
  ...
  await page.locator("#workupStudioSectionTabs button", { hasText: "History questions" }).click();
  await page.waitForSelector("#workupStudioItemLabelInput");
  const historyListAfterDeleteReload = await page.textContent("#workupStudioItemList");
  assert.doesNotMatch(historyListAfterDeleteReload || "", new RegExp(addedHistoryQuestion.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), "Removed history item should stay removed after reload.");
