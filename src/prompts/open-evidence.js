...
export function buildInitialAdmissionPrompt({ patient, guidelines }) {
  ...
  return `Write a concise, chart-ready initial admission note from this de-identified information. Scale detail to case complexity and make a straightforward case very brief. Omit every subjective, objective, and historical detail that does not change the current differential, management, plan, risk, or disposition. Limit the overall Assessment to two sentences and include why the patient was admitted plus the one or two past medical-history conditions most pertinent to the admission. Put all clinical reasoning, supporting discussion, uncertainty, and differential diagnoses under the applicable problem in Plan, not in the overall Assessment. For every new symptom or finding with an unresolved, management-relevant cause, include a Differential block with Most likely diagnosis, Diagnosis 1, Diagnosis 2, Diagnosis 3, Diagnosis 4, and a one- or two-sentence Why most likely comparison based only on chart-supported findings. Do not repeat information.
