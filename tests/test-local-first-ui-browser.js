...
try {
  ...
  assert.match(await page.locator('[data-demo-guide]').innerText(), /Add a patient/i);
  ...
  await page.waitForSelector('[data-demo-guide]');
  assert.match(await page.locator('[data-demo-guide]').innerText(), /Add a patient/i);
  assert.equal(await page.locator("#newPatientLabel").isVisible(), true);
  assert.match(await page.locator("#newPatientLabel").inputValue(), /Synthetic demo/);
  assert.equal(await page.locator('[data-action="add-demo-patient"]').count(), 1, "the guided demo must expose its dedicated Add patient action");
  assert.equal(await page.locator('[data-action="admit-patient"]').count(), 0, "the normal roster action must not be available during the demo handoff");
  await page.click('[data-action="add-demo-patient"]');
  await page.waitForSelector('[data-action="save-context"]');
  assert.equal(await page.locator("#patientSwitcher option").count(), 1, "the guided demo must replace its temporary roster state with one synthetic patient");
  await page.click('[data-action="save-context"]');
  await page.waitForSelector('#contextSections .redaction-review');
  assert.match(await page.locator('[data-demo-guide]').innerText(), /Confirm rest/i);
  await page.click('[data-action="exit-guided-demo"]');
  await page.waitForFunction(() => /Guided demo closed/.test(document.querySelector("#statusLine")?.textContent || ""));
