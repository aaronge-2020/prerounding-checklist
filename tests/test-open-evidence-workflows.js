...
assert.equal(openEvidenceTasks.length, 7, "OpenEvidence tasks should include rounds, safety, teaching, discharge readiness, and attending discharge plan tasks");
...
const outputContracts = {
  ...
  attending_discharge_plan: ["MEDICATIONS", "FOLLOW-UP", "RETURN PRECAUTIONS", "inline citation to current guideline or literature", "Do not return JSON"]
};
...
openEvidenceTasks.forEach((task) => {
  ...
  assert.ok(built.prompt.length < 5200, `${task.id} base prompt should stay compact enough to review; saw ${built.prompt.length} chars`);
...
assert.ok(!ids.has("find_exception"), "find-exception task should be removed with the cut workup-tree features");
assert.ok(!ids.has("attending_plan"), "attending-level plan task should be replaced by the attending discharge plan task");
assert.ok(!ids.has("what_am_i_missing"), "blind-spot review task should be removed with the cut workup-tree features");
...
["initial_rounds_report", "full_rounds_report", "final_rounds_update", "attending_discharge_plan"].forEach((taskId) => {
...
const attendingDischargePlanPrompt = buildOpenEvidencePrompt("attending_discharge_plan", baseContext);
assert.equal(attendingDischargePlanPrompt.requiredContext, "source", "attending discharge plan should require de-identified source context");
assert.equal(attendingDischargePlanPrompt.outputKind, "discharge_plan", "attending discharge plan should use the discharge_plan output kind");
assert.ok(attendingDischargePlanPrompt.prompt.includes("MEDICATIONS"), "attending discharge plan should include a medications section");
assert.ok(attendingDischargePlanPrompt.prompt.includes("assuming discharge criteria are met"), "attending discharge plan should assume discharge readiness rather than assess it");
assert.ok(attendingDischargePlanPrompt.prompt.includes("assessing whether the patient is ready for discharge"), "attending discharge plan should exclude discharge-readiness assessment from scope");
...
const attendingDischargePlanResult = parseOpenEvidenceResult({
  taskId: "attending_discharge_plan",
  outputKind: "discharge_plan",
  text: "MEDICATIONS\n- Continue insulin strategy supported by Diabetes Care 2024. doi:10.2337/dci24-0032"
});
assert.ok(attendingDischargePlanResult.sections.some((section) => /MEDICATIONS/.test(section.title)), "attending discharge plan parser should retain section headings");
assert.ok(extractCitations(attendingDischargePlanResult.sections.map((section) => section.body).join("\n")).length >= 1, "citation extractor should find guideline citations in attending discharge plan output");
