...
assert.match(admission, /Make the overall Assessment exactly one shorter sentence/);
...
assert.match(guidelines.admission, /Every selective fact in HPI and Objective must support a named problem/);
...
assert.match(progress, /Make the overall Assessment exactly one sentence/);
...
assert.match(guidelines.progress, /silently review the complete documented selected-day examination/);
...
assert.match(guidelines.progress, /interval gate does not apply to the standard 24-hour vital-sign summary/i);
