...
assert.ok(html.includes('id="workupStudioMagicLinkEmailInput"'), "Workup Studio should ask only for an email address for magic-link sign-in.");
assert.ok(html.includes("Send magic link"), "Workup Studio should use magic-link auth instead of username/password sign-in.");
assert.ok(html.includes("createClient as createSupabaseClient"), "Workup Studio should use the official supabase-js client for auth.");
assert.ok(html.includes("flowType: \"pkce\""), "Supabase client should use the PKCE auth flow.");
assert.ok(html.includes("detectSessionInUrl: true"), "Supabase client should auto-detect magic-link sign-in redirects.");
...
assert.ok(html.includes("shouldCreateUser: false"), "Magic links must require an existing Supabase Auth user.");
assert.ok(html.includes("workupStudioSupabase.auth.onAuthStateChange"), "Workup Studio should react to supabase-js auth state changes.");
assert.ok(html.includes("workupStudioSupabase.auth.getSession()"), "Workup Studio should read the current session through supabase-js.");
assert.ok(html.includes("workupStudioSupabase.auth.signOut()"), "Sign out should use supabase-js.");
...
assert.ok(html.includes("`${window.location.origin}${window.location.pathname || \"/\"}`"), "Magic-link redirects should use a stable callback path.");
assert.ok(html.includes("Enter a valid assigned account email"), "Invalid magic-link email errors should be understandable.");
...
assert.ok(html.includes("authorization: `Bearer ${session.access_token}`"), "REST calls must use a real user access token from the Supabase session.");
assert.ok(!html.includes("authorization: `Bearer ${session.access_token || workupStudioState.backend.anonKey}`"), "REST calls must not fall back to publishable key auth.");
