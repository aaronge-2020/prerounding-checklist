...
assert.ok(priorMonthYearAnchorResult.text.includes("Prior transplant evaluation in [1 year and 2 months prior to hospital admission]"), "prior month/year should collapse to a relative duration before admission, never a calendar date");
assert.ok(priorMonthYearAnchorResult.text.includes("PET stress in [1 year and 1 month prior to hospital admission]"), "month/year history should collapse to a relative duration before admission");
...
assert.ok(epicRespImagingResult.text.includes("Oral/Maxillo Panorex Image By Clinic XR: Rpt [21 days prior to hospital admission]"), "imaging title containing Clinic should not be redacted as an organization, and an old date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("XR Metal Screening MR Clearance (Abd): Rpt [20 days prior to hospital admission]"), "XR metal screening title should not be redacted as a name");
...
assert.ok(epicRespImagingResult.text.includes("MR Shoulder W+WO Con RIGHT: Rpt [1 month and 10 days prior to hospital admission]"), "MR Shoulder W+WO title should be preserved and its prior date collapsed to a relative duration before admission");
...
assert.ok(epicRespImagingResult.text.includes("Special Send Out Test: Rpt [22 days prior to hospital admission]"), "special send-out test should not be redacted as a facility");
...
assert.ok(epicRespImagingResult.text.includes("Vitamin B-12: 322 [26 days prior to hospital admission]"), "Vitamin B-12 should not be redacted as a name, and an older date should collapse to a relative duration before admission");
...
assert.ok(epicRespImagingResult.text.includes("Hypochromasia: Moderate ! [1 month and 9 days prior to hospital admission]"), "morphology value with an old date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("Microcytes: Moderate ! [1 month and 9 days prior to hospital admission]"), "microcyte morphology row with an old date should collapse to a relative duration before admission");
...
assert.ok(epicRespImagingResult.text.includes("QuantiFERON TB1 Ag Value: 0.04 [1 month and 4 days prior to hospital admission]"), "QuantiFERON Ag Value should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("Group A Streptococcus Screen w/o Reflex: Rpt [1 month and 3 days prior to hospital admission]"), "strep screen w/o reflex should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("XR Ankle Min 3 Views Each Bilateral: Rpt (E) [1 month and 2 days prior to hospital admission]"), "bilateral ankle imaging title should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("XR Hands 1 or 2 Views Each Bilateral: Rpt (E) [1 month and 1 day prior to hospital admission]"), "bilateral hand imaging title should not be redacted as a name");
...
assert.ok(epicRespImagingResult.text.includes("Manual Diff?: Yes [1 month and 8 days prior to hospital admission]"), "manual differential result should collapse an old date to a relative duration before admission, not a time bucket");
...
assert.ok(epicRespImagingResult.text.includes("Antigen Typing: K Neg  |  C Neg  |  E Neg [1 month and 5 days prior to hospital admission]"), "blood-bank antigen shorthand should not create residual name warnings");
...
assert.ok(epicRespImagingResult.text.includes("Quantiferon TB Gold: Indeterminate ! [1 month and 5 days prior to hospital admission]"), "Quantiferon result with an old date should collapse to a relative duration before admission, not a time bucket");
...
assert.ok(epicRespImagingResult.text.includes("Dolutegravir Resistance: NOT PREDICTED [1 month and 3 days prior to hospital admission]"), "HIV resistance rows with an old date should collapse to a relative duration before admission");
...
assert.ok(epicRespImagingResult.text.includes("POC SARS COV-2 (COVID-19) Ag: Negative [30 days prior to hospital admission]"), "COVID-19 should remain a clinical label token, not an ID placeholder");
