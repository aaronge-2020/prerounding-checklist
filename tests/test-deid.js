...
assert.ok(sameMorningLabTimeResult.text.includes("[Lab 1/2] BMP Collected: [Historical: 2026 at 06:12]"), "first same-morning BMP should be ordinal-tagged and keep its exact time with generic historical semantics");
assert.ok(sameMorningLabTimeResult.text.includes("**[LATEST_RESULT]** [Lab 2/2] BMP Collected: [Historical: 2026 at 07:45]"), "second, later same-morning BMP should be tagged as the latest result and keep its exact time");
...
assert.ok(pocGlucoseTimeResult.text.includes("POC Glucose [Historical: 2026 at 04:02] 160"), "POC glucose exact AM time should be preserved with generic historical semantics");
assert.ok(pocGlucoseTimeResult.text.includes("POC Glucose [Historical: 2026 at 08:01] 190"), "second POC glucose exact AM time should be preserved with generic historical semantics");
...
assert.ok(narrativeTimeResult.text.includes("Symptoms changed on [Historical: 2026 at 06:12]"), "non-result narrative timestamp should preserve exact time with generic historical semantics");
assert.ok(narrativeTimeResult.text.includes("Plan updated after rounds on [Historical: 2026 at 07:45]"), "second non-result narrative timestamp should preserve exact time");
...
// Without an admission anchor the app must not invent a hospital-day
// timeline: multi-day unanchored panels fall back to the generic historical
// placeholder (exact clock times are still preserved).
assert.ok(epicResultsReviewResult.text.includes("WBC: 9.1 [Historical: 2026 at 04:48]"), "latest Results Review row should use the generic historical placeholder and preserve exact clock time");
assert.ok(epicResultsReviewResult.text.includes("Sodium: 140 [Historical: 2026 at 04:48]"), "BMP-like row should preserve exact clock time");
assert.ok(epicResultsReviewResult.text.includes("POC Glucose: 123 [Historical: 2026 at 20:42]"), "same-day/previous-day POC glucose row should preserve exact clock time");
assert.ok(!/Hospital Day \d+/.test(epicResultsReviewResult.text), "unanchored multi-day panel must not fabricate Hospital Day numbers");
assert.ok(epicResultsReviewResult.text.includes("Troponin T 5th Gen, High Sensitivity: 12 [Historical: 2026 at 11:35]"), "high-sensitivity troponin label should not be redacted as a name");
assert.ok(epicResultsReviewResult.text.includes("BK Virus Quant Log 10: Cannot Be Determined [Historical: 2026 at 10:20]"), "Cannot Be Determined should remain a result value");
assert.ok(epicResultsReviewResult.text.includes("EBV LOG10: Cannot Be Determined [Historical: 2026 at 11:35]"), "viral result values should not be name-redacted");
assert.ok(epicResultsReviewResult.text.includes("Parainflu 1 Virus RNA Amplification: Not Detected [Historical: 2026 at 18:42]"), "viral panel label should be preserved");
assert.ok(epicResultsReviewResult.text.includes("Rhinovirus/Enterovirus RNA Amplification: Not Detected [Historical: 2026 at 18:42]"), "slash-separated viral label should be preserved");
assert.ok(epicResultsReviewResult.text.includes("XR Knee 1/2 Views RIGHT: Rpt (E) [Historical: 2026 at 07:31]"), "imaging view count should not be converted to a date");
assert.ok(epicResultsReviewResult.text.includes("US Renal Transplant W/Duplex: Rpt [Historical: 2026 at 20:43]"), "renal transplant imaging result should not be location/name redacted");
assert.ok(epicResultsReviewResult.text.includes("CAR Echo 2D Adult Complete: Rpt [Historical: 2026 at 10:06]"), "Adult Complete echo label should be preserved");
assert.ok(epicResultsReviewResult.text.includes("QTC Calculation (Bazett): 453 [Historical: 2026 at 23:41]"), "QTC Calculation should not be redacted as a name");
assert.ok(epicResultsReviewResult.text.includes("External Hemoglobin A1C: 8.2 (H) (E) [Historical: 2026 at 07:43]"), "External Hemoglobin A1C should remain clinical result text");
assert.ok(epicResultsReviewResult.text.includes("Miscellaneous Test: SEE LAB TAB [Historical: 2026 at 12:00]"), "stray SEE LAB TAB should not become an unrelated label");
assert.ok(epicResultsReviewResult.text.includes("Transfuse Red Blood Cells: Rpt  |  Rpt [Historical: 2026 at 10:28]\n\n[Historical: 2026 at 21:31]\nPOC Glucose: 160"), "date conversion should not merge lines across blank lines");
assert.ok(epicResultsReviewResult.text.includes("[Historical: 2026 at 05:32]\nPOC Glucose: 95"), "standalone POC glucose header should preserve exact time");
assert.ok(epicResultsReviewResult.text.includes("[Historical: 2026 at 05:51]\nPOC Glucose: 150"), "multiple AM POC glucose headers should stay distinguishable");
assert.ok(epicResultsReviewResult.text.includes("[Historical: 2026 at 11:54]\nPOC Glucose: 111"), "POC glucose value should not be rewritten as a date placeholder");
assert.ok(epicResultsReviewResult.text.includes("[Historical: 2026 at 04:48]\nWBC: 9.1"), "standalone CBC/BMP header should preserve exact collection/result time");
...
assert.ok(epicRespImagingResult.text.includes("[Lab 1/2] BG Site: A-Line [Historical: 2026 at 15:49]"), "A-line blood gas site should remain clinical text, keep exact time, and be ordinal-tagged as the first of its repeated label");
assert.ok(epicRespImagingResult.text.includes("pH, Arterial: 7.35 [Historical: 2026 at 10:42]"), "blood gas result should preserve exact clock time");
assert.ok(epicRespImagingResult.text.includes("Oxygen Device: Ventilator [Historical: 2026 at 09:54]"), "vent setting row should preserve exact clock time");
assert.ok(epicRespImagingResult.text.includes("Mode: High Flow Nasal Cannula [Historical: 2026 at 20:49]"), "respiratory mode should not be treated as a name");
assert.ok(epicRespImagingResult.text.includes("WBC: 8.8 [Historical: 2026 at 12:22]"), "CBC row should use the generic historical placeholder and preserve exact clock time");
assert.ok(epicRespImagingResult.text.includes("POC Glucose: 104 [Historical: 2026 at 08:10]"), "latest POC glucose should use the generic historical placeholder and preserve exact clock time");
assert.ok(epicRespImagingResult.text.includes("Card 1 Collection Date: [Historical: 2026] [Historical: 2026 at 10:28]"), "Card 1 Collection Date should not leak a calendar date and should not duplicate exact-time wording");
assert.ok(epicRespImagingResult.text.includes("Oral/Maxillo Panorex Image By Clinic XR: Rpt [Historical: 2026 at 17:39]"), "imaging title containing Clinic should not be redacted as an organization, and an old date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("XR Metal Screening MR Clearance (Abd): Rpt [Historical: 2026 at 15:42]"), "XR metal screening title should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("IR CV Tunneled Cath Insertion >=5 yrs: Rpt [Historical: 2026 at 14:33]"), "IR tunneled catheter title should be preserved");
assert.ok(epicRespImagingResult.text.includes("MR Shoulder W+WO Con RIGHT: Rpt [Historical: 2025 at 06:30]"), "MR Shoulder W+WO title should be preserved and its prior date collapsed to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("Request for: Plasma: Rpt [Historical: 2026 at 10:55]"), "blood product request rows should preserve exact clock time");
assert.ok(epicRespImagingResult.text.includes("Transfuse Platelets: Rpt  |  Rpt [Historical: 2026 at 20:43]"), "transfusion rows should preserve exact clock time");
assert.ok(epicRespImagingResult.text.includes("Special Send Out Test: Rpt [Historical: 2026 at 13:10]"), "special send-out test should not be redacted as a facility");
assert.ok(epicRespImagingResult.text.includes("HCV Spec Site: Blood [Historical: 2026 at 08:51]"), "HCV spec site value Blood should remain clinical text");
assert.ok(epicRespImagingResult.text.includes("Iron: Not Performed [Historical: 2026 at 06:19]"), "Not Performed should remain a result value");
assert.ok(epicRespImagingResult.text.includes("Vitamin B-12: 322 [Historical: 2026 at 04:20]"), "Vitamin B-12 should not be redacted as a name, and an older date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("**[LATEST_RESULT]** [Lab 2/2] BG Site: Peripheral Venous [Historical: 2026 at 03:40]"), "Peripheral Venous should remain a blood gas site value and be tagged as the most recent of its repeated label");
assert.ok(epicRespImagingResult.text.includes("Base Exc Art: -4.1 [Historical: 2026 at 16:02]"), "arterial base excess should preserve exact result time");
assert.ok(epicRespImagingResult.text.includes("Base Excess, Ven: -1.8 [Historical: 2026 at 03:40]"), "venous base excess should preserve exact result time");
assert.ok(epicRespImagingResult.text.includes("Hypochromasia: Moderate ! [Historical: 2026 at 21:16]"), "morphology value with an old date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("Microcytes: Moderate ! [Historical: 2026 at 21:16]"), "microcyte morphology row with an old date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("Retic CT %: 1.23 [Historical: 2026 at 21:49]"), "Retic CT should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("Body Fluid Culture (Aerobic): Rpt !! [Historical: 2026 at 13:38]"), "body fluid culture should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("QuantiFERON TB1 Ag Value: 0.04 [Historical: 2026 at 09:40]"), "QuantiFERON Ag Value should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("Group A Streptococcus Screen w/o Reflex: Rpt [Historical: 2026 at 09:48]"), "strep screen w/o reflex should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("XR Ankle Min 3 Views Each Bilateral: Rpt (E) [Historical: 2026 at 14:56]"), "bilateral ankle imaging title should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("XR Hands 1 or 2 Views Each Bilateral: Rpt (E) [Historical: 2026 at 14:45]"), "bilateral hand imaging title should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("CAR Echo 2D w Agitated Saline Adult Comp: Rpt (C) [Historical: 2026 at 10:45]"), "agitated saline echo title should not be redacted as a patient name");
assert.ok(epicRespImagingResult.text.includes("POCUS ANES Procedure Nerve or Fascial Plane Block: Rpt [Historical: 2026 at 10:26]"), "POCUS anesthesia fascial-plane block title should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("Manual Diff?: Yes [Historical: 2026 at 11:19]"), "manual differential result should collapse an old date to a relative duration before admission, not a time bucket");
assert.ok(epicRespImagingResult.text.includes("Ref Com Comments: H/O Anti-K; No other antibody detected. [Historical: 2026 at 07:59]"), "blood-bank reference comments should keep exact time and not create name warnings");
assert.ok(epicRespImagingResult.text.includes("TACROLIMUS BLOOD (FK506), LC/MS/MS (T): 3.4 [Historical: 2026 at 05:12]"), "Tacrolimus blood label should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("[Lab 1/2] ABO Rh: B Negative [Historical: 2026 at 07:00]"), "blood type value B Negative should not create residual name warnings and should be ordinal-tagged");
assert.ok(epicRespImagingResult.text.includes("Antigen Typing: K Neg  |  C Neg  |  E Neg [Historical: 2026 at 14:06]"), "blood-bank antigen shorthand should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("Spec Gravity, Ur: 1.030 [Historical: 2026 at 20:08]"), "urine specific gravity should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("Leukocyte Esterase, Ur: Trace ! [Historical: 2026 at 20:08]"), "leukocyte esterase should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("CT Head/Brain W/O Con: Rpt [Historical: 2026 at 19:06]"), "CT head/brain W/O contrast title should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("VAS Venous Duplex LE Bilat Complete: Rpt [Historical: 2026 at 11:58]"), "LE bilateral venous duplex title should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("POC SAT O2 ART: 98 [Historical: 2026 at 12:08]"), "POC saturation label should not create residual name warnings");
assert.ok(epicRespImagingResult.text.includes("HCG Beta Subunit - QuaL: Negative [Historical: 2026 at 06:19]"), "HCG beta subunit label should remain clinical text");
assert.ok(epicRespImagingResult.text.includes("Quantiferon TB Gold: Indeterminate ! [Historical: 2026 at 15:54]"), "Quantiferon result with an old date should collapse to a relative duration before admission, not a time bucket");
assert.ok(epicRespImagingResult.text.includes("TREPONEMA PALLIDUM IGG AB: Minimal React [Historical: 2026 at 03:14]"), "Treponema serology should keep exact result time");
assert.ok(epicRespImagingResult.text.includes("Bacterial Vaginosis, RNA: Positive ! [Historical: 2026 at 12:00]"), "Bacterial Vaginosis RNA should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("Dolutegravir Resistance: NOT PREDICTED [Historical: 2026 at 15:02]"), "HIV resistance rows with an old date should collapse to a relative duration before admission");
assert.ok(epicRespImagingResult.text.includes("Antibody Identification: Cold Autoantibody [Historical: 2026 at 06:19]"), "cold autoantibody result value should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("DIRECT COOMBS-C3D: Negative [Historical: 2026 at 14:26]"), "COOMBS-C3D should remain a clinical label, not an ID placeholder");
assert.ok(epicRespImagingResult.text.includes("Tissue Examination: Rpt (P) [Historical: 2026 at 10:43]"), "tissue examination label should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("POC SARS COV-2 (COVID-19) Ag: Negative [Historical: 2026 at 04:02]"), "COVID-19 should remain a clinical label token, not an ID placeholder");
assert.ok(epicRespImagingResult.text.includes("Test Cup: Plain Cup [Historical: 2026 at 15:00]"), "TEG cup descriptor should keep exact result time");
assert.ok(epicRespImagingResult.text.includes("Appearance, Ur: Slightly Cloudy [Historical: 2026 at 18:57]"), "urine appearance descriptor should keep exact result time");
assert.ok(epicRespImagingResult.text.includes("R Axis: -2 [Historical: 2026 at 18:12]"), "EKG axis result should keep exact result time");
assert.ok(epicRespImagingResult.text.includes("Banding Type: FISH [Historical: 2026 at 02:31]"), "cytogenetics banding row should keep exact result time");
assert.ok(epicRespImagingResult.text.includes("Source: Blood\n [Historical: 2026 at 02:31]"), "standalone timestamp after specimen source should keep exact result time");
assert.ok(epicRespImagingResult.text.includes("TEG Platelet Aggregation (MA): 68.3 [Historical: 2026 at 15:00]"), "TEG platelet aggregation should not be redacted as a name");
assert.ok(epicRespImagingResult.text.includes("Beta HCG Tumor Marker (Male): 2080 (H) [Historical: 2026 at 04:33]"), "tumor-marker lab label should not be redacted as a patient name");
assert.ok(epicRespImagingResult.text.includes("Prelim Result: Normal FISH [Historical: 2026 at 02:31]"), "Normal FISH result should remain clinical text");
assert.ok(epicRespImagingResult.text.includes("[Historical: 2026 at 07:51]\nPOC Glucose: 101"), "timestamped POC glucose block should keep exact clock time");
...
// User-reported Quick De-ID regressions: a bare street name must be an
// ADDRESS (never a provider/person name), and a medical abbreviation in
// all caps ("Retic CT", "Chest CT") must not become an ADDRESS.
const mainStreetResult = deidentifyTextStructuredOnly("Patient lives on Main St near the hospital.", null);
assert.ok(mainStreetResult.text.includes("lives on [ADDRESS]"), "bare street name Main St should be labeled ADDRESS");
assert.ok(!mainStreetResult.text.includes("[PROVIDER NAME]"), "Main St must not be labeled a provider name");
const mainStreetFullResult = deidentifyTextStructuredOnly("Follow up with Dr Smith on Main Street.", null);
assert.ok(mainStreetFullResult.text.includes("[PROVIDER NAME] on [ADDRESS]"), "Main Street should be ADDRESS while Dr Smith stays a provider name");
const reticCtResult = deidentifyTextStructuredOnly("Retic CT %: 1.23", null);
assert.ok(!reticCtResult.text.includes("[ADDRESS]"), "Retic CT (computed tomography) must not be labeled an address");
const chestCtResult = deidentifyTextStructuredOnly("Chest CT showed no acute findings.", null);
assert.equal(chestCtResult.text, "Chest CT showed no acute findings.", "Chest CT must remain untouched clinical text");

// User-reported Quick De-ID regression: unanchored dates spanning multiple
// days must use the generic historical placeholder - never a fabricated
// Hospital Day count. A single-date cluster (e.g. a timestamped lab panel)
// with no admission context gets the same generic historical semantics with
// its exact clock time preserved - it must not anchor itself to a fictitious
// "Hospital Day 1". True admission context (or an explicit caller date)
// still anchors real Hospital Days.
const unanchoredMultiDayResult = deidentifyTextStructuredOnly("ED note: seen 09/18/2026, recheck 09/20/2026.", null);
assert.ok(!/Hospital Day \d+/.test(unanchoredMultiDayResult.text), "unanchored multi-day dates must not fabricate Hospital Day numbers");
assert.ok(unanchoredMultiDayResult.text.includes("[Historical: 2026]"), "unanchored multi-day dates should use the generic historical placeholder");
const singleDayPanelResult = deidentifyTextStructuredOnly("POC Glucose 06/06/2026 04:02 160\nPOC Glucose 06/06/2026 08:01 190", null);
assert.ok(singleDayPanelResult.text.includes("[Historical: 2026 at 04:02]"), "single-day panel without admission context must use generic historical semantics with exact clock time, never Hospital Day 1");
assert.ok(!/Hospital Day \d+/.test(singleDayPanelResult.text), "single-day panel must not fabricate a Hospital Day number");
const anchoredAdmissionResult = deidentifyTextStructuredOnly("Admitted on 09/18/2026. Labs drawn 09/20/2026 were normal.", null);
assert.ok(anchoredAdmissionResult.text.includes("Admitted on [Hospital Day 1]"), "admission date should anchor Hospital Day 1");
assert.ok(anchoredAdmissionResult.text.includes("Labs drawn [Hospital Day 3]"), "later same-admission date should advance the Hospital Day count");
// "Previously admitted" names a prior stay, not this admission: neither it
// nor a "current labs" reading-date cue may invent a hospital admission
// timeline. Both dates degrade to generic historical semantics.
const priorAdmissionResult = deidentifyTextStructuredOnly("Previously admitted on 09/18/2025. Current labs 09/20/2026 normal.", null);
