...
const genericPattern = /\b(?:any red flag, unstable vital sign, or emergency feature|required diagnostic data available|initial assessment: stabilize|collect source-directed confirmation|severity or disposition-changing risk present|diagnostic criteria met or treatment cannot wait|source-backed|selected workup|first-line management only|use the high-risk or confirmed-pathway management option|lower-risk, outpatient, supportive, or safety-net pathway only|clinician review flagged|local policy and patient-specific clinician review flagged|explicit guideline cutoffs|cutoff-bearing|cutoff criteria|trigger present|enough context|gather objective data|repeat critical [^.]+ objective data|(?:the|an) abnormal cutoff|using the abnormal cutoff|according to the abnormal cutoff|danger features?|crisis labs|obtain targeted tests|targeted tests and cultures|concurrent data bundle obtained|criteria match|does patient context support this workup|treatment modifier unresolved|treatment started or diagnostic plan active|worsening or discordant reassessment|stopping or de-escalation criteria met|stable for follow-up with safety net|diagnosis\/risk branch selected|threshold\/result data missing|Missing data needed|Missing data:|Missing context:|Cannot distinguish|Cannot classify|cannot [^.;]+ until [^.;]+ known|through missing-data|from missing data|Choose missing-data|Choose from missing [^,]+|branch-specific criteria in this node apply|criterion-defining results|documented disease branch|source-directed|route by findings|route by|compact evidence-cited management pathway|treatment branch only when the cited diagnostic\/severity criteria match|key thresholds and interpretation caveats|guideline-sourced endocrine workup|threshold-defined branch|presenting trigger|assessment and key results|key labs\/results|key results|threshold labs\/results|threshold results|classify risk and disposition|treatment safety resolved|clinician review modifier|local protocol dependency|objective findings favor|active branch|Interpret with local lab ranges|Reference anchors|Patient-specific contraindication, special population|Competing diagnosis or exclusion changes the active pathway|initial data bundle|choose clinical branch|escalation branch|treatment\/disposition plan|treatment safety review|monitoring and readiness|reassessment escalation|de-escalation or stopping criteria|alternate diagnosis pathway|triage inputs|severity thresholds|treatment\/disposition|contraindication screen|response monitoring|deterioration criteria|mimic\/exclusion|Assessment bundle:|Treat\/dispose using confirmed category|Medication, procedure, pregnancy, comorbidity, dose, or order-set modifier|active management|specialist-input modifier|named alternate diagnosis|management branch|management changes when|document the exact modifier|criteria are absent or contradicted by|competing diagnosis must be named|switch to [^.]+ when it fits better|Threshold route:|Closure: result owner|treatment node|care-location checks|record the reviewer recommendation|protocol used|matching workup|Emergency stabilization route|RBC transfusion threshold route|Iron therapy and diet route|ITP observation treatment and precautions route|Leukemia lymphoma urgent referral route|Hematology handoff and review owner route)\b/i;
