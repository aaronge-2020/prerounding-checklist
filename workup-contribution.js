...
function normalizeContributionItem(item = {}, sectionKey = "", expectedType = "", index = 0, options = {}) {
  ...
  if (rawType === "diagnostic_test") {
    normalized.test_name = firstString(item.test_name, item.test, item.label, item.action);
    normalized.order_when = firstString(item.order_when, item.when, item.trigger, "When this workup is clinically relevant.");
    normalized.diagnostic_target = firstString(item.diagnostic_target, item.diagnostic_purpose);
    normalized.interpretation = firstString(item.interpretation, item.rationale);
    normalized.result_changes_management = firstString(normalized.result_changes_management, item.management_change, item.management_implication);
    normalized.threshold_ids = stringArray(item.threshold_ids);
  }
...
export function validateWorkupContributionDraft(input = {}, options = {}) {
  ...
  for (const [sectionKey, expectedType] of sectionDefinitions) {
    ...
    for (const [index, item] of draft[sectionKey].entries()) {
      ...
      if (item.item_type !== expectedType && sectionKey !== "management_disposition") {
  ...
  if (!itemCount) issues.push("missing workup items: add at least one history, exam, test, safety, red flag, or management item");
  return {
    ok: issues.length === 0,
    issues,
    draft
